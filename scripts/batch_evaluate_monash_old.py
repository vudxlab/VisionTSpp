...
for line in lines[st:ed]:
    ...
    if os.path.exists(line.replace(".ckpt", "_pf_quantile.csv")):
        continue
    ...
    print("quantile_int:", quantile_int)
    # 非quantile的就不需要重跑一遍了～
    if not quantile:
        continue
    ...
    for ds in datasets_pf:
        cmd = [
            ...
            "--save_name", line.replace(".ckpt", "_pf_quantile.csv"),
            "--periodicity", "freq", 
            "--context_len", "2000",
            "--batch_size", "256",
...
for line in lines[st:ed]:
    ...
    if os.path.exists(line.replace(".ckpt", "_quantile.csv")):
        continue
    ...
    for ds in datasets:
        cmd = [
            ...
            "--save_name", line.replace(".ckpt", "_quantile.csv"),
...
for line in lines[st:ed]:
    ...
    for pred_len in ltsf_pred_len_list:
        ...
        runned_dataset_list = []
        if os.path.exists(line.replace(".ckpt", f"_ltsf_pred{pred_len}_quantile.csv")):
            with open(line.replace(".ckpt", f"_ltsf_pred{pred_len}_quantile.csv"), "r") as f:
                for content in f:
                    if content.strip() == "": 
                        continue
        ...
        for ds in datasets_ltsf:
            ...
            if ds == "ETTh1" or ds == "ETTh2":
                if pred_len == 96:
                    ...
                    if not 'epoch=999' in line: 
                        continue
            if ds == "ETTm1" or ds == "ETTm2" or ds == "Electricity" or ds == "Weather":
            ...
            cmd = [
                ...
                "--save_name", line.replace(".ckpt", f"_ltsf_pred{pred_len}_quantile.csv"),
                "--quantile", str(quantile_int),
            ]
            print(cmd)
