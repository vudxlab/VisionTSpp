...
for root, dirs, files in os.walk("../outputs"):
    for file in files:
        try:
            ...
            # 250322 adds:
            if file.endswith("_pf_quantile.csv") and 'last' not in file:
                csv_path = os.path.join(root, file)
                raw_result = pd.read_csv(csv_path)['MASE[0.5]']
                result = np.asarray(raw_result)
                # result = np.round(result, 2)
                ...
                step = file.split("-step=")[-1].split(".csv")[0]
                name = root.split("/")[-2]
                if step.endswith("_pf_quantile"):
                    step = step.replace("_pf_quantile", "")

                if step == "0":
                    print(float((result / naive_result).prod() ** (1.0 / len(result))))
                    print(result.tolist())
                    continue
                if name not in name_step_result_emsemble:
                    name_step_result_emsemble[name] = {}
                name_step_result_emsemble[name][step] = [
                    float((result / naive_result).prod() ** (1.0 / len(result))),
                    (result > zero_shot).mean()
                ]
        except: 
...
# 250322 adds: 
for deleted in list(name_step_result_emsemble.keys()):
    print(deleted)
    if 'quantile' not in deleted or deleted not in showed:
        del name_step_result_emsemble[deleted]
...
# 250322 adds:
for n in name_step_result_emsemble:
    # if n == 'first_run' or 'unweigh' in n or 'full' in n: continue
    if 'rd' not in n:
        y = [0.8374937152805807]
        x = [0]
        ...
        y = []
        x = []
    for step, result in sorted(name_step_result_emsemble[n].items(), key=lambda x: int(x[0])):
        x.append(int(step))
        # y.append(result[0])
        y.append(max(result[0], 0.7))
    print(n, y)
    plt.plot(x, y, label=n+'_emsemble', markersize=5, marker='o')
