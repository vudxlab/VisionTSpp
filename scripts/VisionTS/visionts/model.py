...
class VisionTS(nn.Module):
    ...
    def forward(self, x, export_image=False, fp64=False, multivariate=False, color_list=None, LOOKBACK_LEN_VISUAL=300):
        ...
        if self.quantile:
            ...
            y_quantile_list = y_pred_quantile_list  # 每个元素的shape为：[bs, pred_len, nvars]
        ...
        if export_image:
            ...
            x_2d_visual = x_2d[:, :, :, -period_num:]  # x_2d.shape: [bs, nvars, 周期长度f, 周期数p] -> 只取出最后
