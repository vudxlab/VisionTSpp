...
def forecast(model: VisionTS, train_list: list, test_list: list, batch_size, device, periodicity, num_patch_input):
    ...
    forecast_np_std = {} # raw index -> forecast
    for train_len in seq_len_to_group_data:
        ...
        model.update_config(context_len=context_len, pred_len=pred_len, periodicity=periodicity, num_patch_input=num_patch_input, padding_mode='constant')
        ...
        for batch_i in range(int(math.ceil(len(cur_train) / batch_size))):
            ...
            if model.logvar:
                cur_batch_pred, cur_batch_pred_std = cur_batch_pred
            for i in range(len(cur_batch_pred)):
                ...
                if model.logvar:
                    forecast_np_std[cur_idx] = cur_batch_pred_std[i, :, 0].detach().cpu().numpy()

    if model.logvar:
        return (
            np.stack([forecast_np[i] for i in range(len(train_list))]), 
            np.stack([forecast_np_std[i] for i in range(len(train_list))]), 
        )
...
def evaluate(
    ...
    model = VisionTS(mae_arch, ckpt_dir=checkpoint_dir, ckpt_path=checkpoint_path, logvar=periodicity in ['autotune_std', 'autotune_globalstd']).to(device)
    ...
    if periodicity in ["autotune_std", "autotune_globalstd"]:
        seasonality_list = freq_to_seasonality_list(metadata.freq, POSSIBLE_SEASONALITIES)
        forecast_values = None
        forecast_values_std = None
        for periodicity in tqdm(seasonality_list, desc='forecast seasonality'):
            ...
            cur_forecast_values, cur_forecast_values_std = forecast(model, train, data_test, batch_size, device, periodicity, num_patch_input) # [B, L]
            cur_forecast_values_std = cur_forecast_values_std.mean(axis=-1, keepdims=False)
            if forecast_values is None:
                forecast_values = cur_forecast_values.copy()
                forecast_values_std = cur_forecast_values_std.copy()
            else:
                if periodicity == 'autotune_std':
                    for i in range(len(forecast_values)):
                        if cur_forecast_values_std[i] < forecast_values_std[i]:
                            forecast_values_std[i] = cur_forecast_values_std[i]
                            forecast_values[i] = cur_forecast_values[i]
                else:
                    if cur_forecast_values_std.mean() < forecast_values_std.mean():
                        forecast_values_std = cur_forecast_values_std.copy()
                        forecast_values = cur_forecast_values.copy()
            # print(periodicity, cur_forecast_values_std.mean(), evaluate_metrics(cur_forecast_values, test_data))
    else:
        if periodicity == "autotune":
            ...
            for periodicity in tqdm(seasonality_list, desc='validate seasonality'):
                ...
                val_forecast = forecast(model, val_train, val_test, batch_size, device, periodicity, num_patch_input)
        ...
        cur_context_len = convert_context_len(context_len, no_periodicity_context_len, periodicity)
        train = [x[-cur_context_len:] for x in data_train]
        forecast_values = forecast(model, train, data_test, batch_size, device, periodicity, num_patch_input)

    metrics_df = evaluate_metrics(forecast_values, test_data)
...
def evaluate_metrics(forecast_values, test_data):
    sample_forecasts = []
