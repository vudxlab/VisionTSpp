...
class MoiraiPretrain(L.LightningModule):
    ...
    def show_image(self, image, title='', cur_nvars=1, cur_color_list=None):
        # image is [H, W, 3]
    ...
    def visualization(self, x, y, y_pred, input_image, reconstructed_image, cur_periodicity, cur_context_len, cur_pred_len, cur_nvars=1, cur_color_list=None, y_pred_std=None, y_pred_quantile_list=None):
        # x: [seq_len, 1/nvars], y & y_pred: [pred_len, 1/nvars]
        # input_image & reconstructed_image: [224, 224, 3]

        # Visualization
        plt.subplot(2, 2, 1)
        self.show_image(input_image, 'input', cur_nvars, cur_color_list)
        ...
        plt.subplot(2, 2, 3)
        self.show_image(reconstructed_image, 'reconstructed', cur_nvars, cur_color_list)
        ...
        if y_pred_quantile_list is not None:
            ...
            for j, y_pred_quantile in enumerate(y_pred_quantile_list):
                ...
                if j < len(y_pred_quantile_list) // 2:
                    ...
                    plt.plot(torch.arange(len(y)) + len(x), y_pred_quantile.cpu(), label=f'pred_{10*(j+2)}', color=f'C{j+2}', alpha=0.5)

        # plt.title(f'forecasting')
        plt.title(f'period={cur_periodicity} ctx={cur_context_len}, pred={cur_pred_len}')
        plt.legend()
        plt.tight_layout()
    ...
    def forward_module(self, module, target, nvars, color_list):
        ...
        # 假设我有image_reconstructed，其shape为[batch x 3 x H=224 x W=224]，表示一组图片数据；同时我有nvars，其shape为[batch, 1]表示变量数；以及color_list，其为list(list)类型，len(color_list)=batch, 每个len(color_list[i])=nvars[i]，color_list[i]中的值只会出现在0，1，2之间，对应于图片颜色通道的那一维。
        # 现在我的任务是：首先对于每个image_reconstructed[i]，我先对其H维度分割成nvars[i]维，每维高度为224/nvars[i]。然后对于第k个高度，也即image_reconstructed[i, :, k*224/nvars[i]:(k+1)*224/nvars[i], :]数据，获取其color维度，也即image_reconstructed[i, color[i][k], k*224/nvars[i]:(k+1)*224/nvars[i], :]。如果nvars[i]无法被224整除，最后则还需要补上image_reconstructed[i, 0, nvars[i]*224/nvars[i]:224, :]的数据。最后将这些数据沿着H维度重新拼回成大小为[batch x 1 x H=224 x W=224]的数据。
        def process_images(image_reconstructed, nvars, color_list):
            """
            Args:
                image_reconstructed: tensor, [batch, 3, 224, 224] 输入图像
                nvars: list, [batch, 1] 每个样本的变量数
                color_list: list of lists 颜色索引列表，每个子列表长度等于对应样本的nvars
            Returns:
            ...
            for i in range(batch_size):
                nvar = nvars[i]  # 当前样本的变量数
                colors = color_list[i]   # 当前样本的颜色列表
        ...
        # 对于主数据处理一下：
        image_reconstructed = module.unpatchify(vision_out) # unpatchify后为：[bs x 3 x H=224 x W=224]
        
        # ! 处理RGB通道
        # y_grey = torch.mean(image_reconstructed, 1, keepdim=True) # [B, 1, H, W]
        # ! 250428 adds:
        y_grey = process_images(image_reconstructed, nvars, color_list) # [B, 1, H, W]
        ...
        if self.hparams.output_dist == 'quantile':
            ...
            y_pred_img_quantile_list = []
            for vision_out_quantile in vision_out_quantile_list:
                image_quantile_reconstructed = module.unpatchify(vision_out_quantile)  # recon: [(bs x nvars) x 3 x H x w]
                y_grey_quantile = process_images(image_quantile_reconstructed, nvars, color_list) # [B, 1, H=224, W=224]
                y_pred_img_quantile = image_quantile_reconstructed[:, :, :, pred_pixel_offset:] # [B, 3, H=224, W_pred=112]
                # append到list里面！！
                y_grey_quantile_list.append(y_grey_quantile)
                y_pred_img_quantile_list.append(y_pred_img_quantile)
        ...
        if not self.hparams.output_dist == 'quantile':
            ...
            return mask, y_logvar, image_reconstructed, y_grey, y_pred_img, y_grey_quantile_list, y_pred_img_quantile_list
    ...
    def forward(
        ...
        if not self.hparams.output_dist == 'quantile':
            mask, y_logvar, image_reconstructed, y_grey, y_pred_img = \
                self.forward_module(self.module, target, nvars, color_list)
        else:
            mask, y_logvar, image_reconstructed, y_grey, y_pred_img, y_grey_quantile_list, y_pred_img_quantile_list = \
                self.forward_module(self.module, target, nvars, color_list)
        ...
        if not self.hparams.output_dist == 'quantile':
            ...
            for quantile in quantiles:
                monitors[f'q{quantile:.2f}_loss'] = []
                monitors[f'q{quantile:.2f}_loss_img'] = []
        ...
        for i in range(len(y_grey)):
            ...
            # 对时间序列计算time series loss：
                ...
                if self.hparams.output_dist == 'quantile':
                    cur_y_grey_quantile_list = []
                    for y_grey_quantile in y_grey_quantile_list:
                        cur_y_grey_quantile = torch.unsqueeze(y_grey_quantile[i], 0)  # [1, 1, H, W]
                        cur_y_grey_quantile_list.append(cur_y_grey_quantile)
                ...
                if self.hparams.output_dist == 'quantile':
                    ...
                    for cur_y_grey in cur_y_grey_quantile_list:
                        y_pred_quantile = self.extract_TS_from_image(cur_y_grey, cur_periodicity, cur_scale_x, cur_pad_left, cur_context_len, cur_pred_len, cur_nvars, cur_pad_down)
                ...
                if self.hparams.output_dist == 'gaussian':
                    ...
                    quantile_num = len(quantiles)
                    
                    # predictions = [y_pred_25, y_pred, y_pred_75]
                    predictions = y_pred_quantile_list[:(quantile_num-1)//2] + \
                        [y_pred] + \
                        y_pred_quantile_list[quantile_num//2:]
                    
                    assert len(predictions) == len(quantiles)
                    # print(f"predictions: {len(predictions)}, {predictions[0].shape}, {predictions[-1].shape}")
                    
                    # 定义加权的权重
                    # w_list = [1/3, 1/3, 1/3]
                    w_list = [1/quantile_num] * quantile_num
            ...
            # if self.global_step % self.log_image_step == 0 and not has_drawn_time_series:
                ...
                # 250307 adds:
                if self.hparams.output_dist == 'quantile':
                    cur_y_pred_quantile_list = []
                    try:
                        for y_pred_quantile in y_pred_quantile_list:
                            cur_y_pred_quantile = y_pred_quantile[0, :, 0].detach().float()
                            cur_y_pred_quantile_list.append(cur_y_pred_quantile)
                    except:
                        # 如果当前batch第一个样本为图像，他是不会生成y_pred_25和y_pred75的
                        # 反正visual用，也不是很重要，直接设置成none就ok了～
                        # cur_y_pred_25, cur_y_pred_y_pred_75 = None, None
                        pass
                else:
                ...
                self.visualization(cur_x[i, :, 0].detach().float(), 
                                    ...
                                    cur_nvars=cur_nvars,
                                    cur_color_list=cur_color_list,
                                    y_pred_std=y_pred_std, 
                                    y_pred_quantile_list=cur_y_pred_quantile_list)
