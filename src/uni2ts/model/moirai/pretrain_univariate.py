...
class MoiraiPretrain(L.LightningModule):
    ...
    def forward(
        ...
        target_img: Float[torch.Tensor, "*batch 2 image_size image_size"],  # 预测窗口真实值、第一维也是时间序列转成的图片，另一维则是mask
        x: Float[torch.Tensor, "*batch seq_len 1"],  # 输入窗口对应的时间序列形式
        y: Float[torch.Tensor, "*batch seq_len 1"],  # 预测窗口对应的时间序列形式
        ...
        means: Float[torch.Tensor, "*batch"],
        stdev: Float[torch.Tensor, "*batch"],
