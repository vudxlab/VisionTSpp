...
from uni2ts.data.dataset import HybridTSImageDataset
...
class HybridTSDatasetBuilder(DatasetBuilder):


    def __init__(self, ts_builder: DatasetBuilder, image_sampling_prob: float, num_patches: int, num_patches_input: int, image_size: int):
        super().__init__()
        self.ts_builder = ts_builder
        self.image_sampling_prob = image_sampling_prob
        self.num_patches = num_patches
        self.num_patches_input = num_patches_input
        self.image_size = image_size

    def build_dataset(self):
        raise ValueError(
            "Do not use HybridTSDatasetBuilder to build datasets, build sub datasets individually instead."
        )

    def load_dataset(
        self, transform_map: dict[Any, Callable[..., Transformation]]
    ) -> Dataset:
        return HybridTSImageDataset(self.ts_builder.load_dataset(transform_map), self.image_sampling_prob, self.num_patches, self.num_patches_input, self.image_size)