...
class PadCollate(Collate):
    ...
    def __call__(self, batch: list[Sample]) -> BatchedSample:
        ...
        # # ! 250425 adds:
        # assert all(
        #     [len(sample[self.target_field][0]) <= self.max_dim for sample in batch]
        # ), f"Sample length must be less than or equal to max_dim ({self.max_dim})"
    ...
    def pad_samples(self, batch: list[Sample]) -> BatchedSample:
        for sample in batch:
            for key in self.seq_fields:
                ...
                #                 (self.max_length - len(sample[key]),) + sample[key].shape[1:],
                #                 sample[key].dtype,
                #             )
                #         ),
                #     ]
                # )
                # ! 250425 adds: 
                # * 1、先沿着seq_len维度padding到指定的max_length=8192长度
                ...
                # * 2、然后再沿着变量nvars维度padding到max_dim=512长度
                sample[key] = np.concatenate([
                        default_convert(sample[key]),
                        default_convert(
                            self.pad_func_map[key](
                                sample[key].shape[:1] + (self.max_dim - sample[key].shape[1],),
                                sample[key].dtype,
                            )
                        ),
                    ], 
                    axis=1,  # 这里是dim=1，因为需要沿着变量nvars维度做cat！
                )
                # * 3、最后再把数据转成tensor
                sample[key] = torch.from_numpy(sample[key])
            
            # ! 250425 adds: 然后说means和stdevs也要padding到max_dim=512长度
            for key in self.seq_fields_mean_stdev:
                sample[key] = torch.cat([
                        default_convert(sample[key]),
                        default_convert(
                            self.pad_func_map[key](
                                (self.max_dim - sample[key].shape[0],),
                                sample[key].dtype,
                            )
                        ),
                    ]
        ...
        # default_collate的目的好像就是把batch中所有数据都先按batch维度合并，然后再转成tensor形式！
