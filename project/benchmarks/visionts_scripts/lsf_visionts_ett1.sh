...
for ds in ETTh1 ETTm1; do
  for cl in 1728 2304 2880 4032; do
    for pl in 96 192 336 720; do
        python run_visionts.py --dataset=${ds}  --run_name=visionts --save_dir=lsf_results --test_setting=lsf --model=visionts --periodicity=freq --context_len=${cl} --pred_length=${pl} --batch_size 128 --split_name=val

        python run_visionts.py --dataset=${ds}  --run_name=visionts --save_dir=lsf_results --test_setting=lsf --model=visionts --periodicity=freq --context_len=${cl} --pred_length=${pl} --batch_size 128 --split_name=test
      done
    done
done
