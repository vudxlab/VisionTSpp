...
def evaluate(
    ...
    if test_setting == "monash" or test_setting == "pf":
        ...
        get_dataset = partial(get_lsf_test_dataset, prediction_length=prediction_length, split_name=split_name)
    else:
    ...
    with open(f"tune_{dataset}.txt", "a+") as f:
        f.write(f"{dataset},{cur_context_len},{prediction_length},{split_name},{metrics_df['MSE[mean]'][0]},{metrics_df['MAE[0.5]'][0]}\n")
    return metrics_df
